...
def model (X, w, w2, w3, w4, w_o, p_keep_conv, p_keep_hidden): 
	ksize,strides = [1,2,2,1], [1,2,2,1]
	l1a = tf.nn.relu(tf.nn.conv2d(X, w, [1, 1, 1, 1], 'SAME'))
		#conv2d takes a 4d input, filter (weight), strides (ints of length 4, the sliding window for each dimension of input), padding (a string from:'SAME', 'VALID', type of padding algorithm to use)
	l1 = tf.nn.max_pool(l1a, ksize, strides, 'SAME')
	l1 = tf.nn.dropout(l1, p_keep_conv)

	l2a = tf.nn.relu(tf.nn.conv2d(l1, w2, [1,1,1,1], 'SAME'))
	l2 = tf.nn.max_pool(l2a, ksize, strides, 'SAME')
	l2 = tf.nn.dropout(l2, p_keep_conv)

	l3a = tf.nn.relu(tf.nn.conv2d(l2, w3, [1,1,1,1], 'SAME'))
	l3 = tf.nn.max_pool(l3a, ksize, strides, 'SAME')
